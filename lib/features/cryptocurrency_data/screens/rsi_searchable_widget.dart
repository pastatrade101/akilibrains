...
import 'package:trade101/features/cryptocurrency_data/controller/rsi_controller.dart';
...
class RsiWidgetSearch extends StatelessWidget {
  final RsiController controller;

  const RsiWidgetSearch({Key? key, required this.controller}) : super(key: key);
    ...
    return TypeAheadField<Crypto>(
      ...
      itemBuilder: (context, Crypto crypto) {
        return Container(
          ...
          color: TColors.black,
          child: ListTile(
            title: Text(crypto.symbol,style: const TextStyle(color: TColors.light),),
          ),
        );
      },
      onSelected: (Crypto crypto) {
        // Do something with the selected crypto
        controller.selectedPair.value = crypto.symbol;
        controller.fetchBtcRsi(crypto.symbol, this.controller.selectedTime.value);
      },
    );
  }

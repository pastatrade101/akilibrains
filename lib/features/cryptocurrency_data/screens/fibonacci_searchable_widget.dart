...
class FibonancciPairSelectionWidgetCrypto extends StatelessWidget {
  final FibonacciRetracementController controller;

  const FibonancciPairSelectionWidgetCrypto({Key? key, required this.controller}) : super(key: key);
    ...
    return TypeAheadField<Crypto>(
      ...
      itemBuilder: (context, Crypto crypto) {
        return Container(
          ...
          color: TColors.black,
          child: ListTile(
            title: Text(crypto.symbol,style: const TextStyle(color: TColors.white),),
          ),
        );
      },
